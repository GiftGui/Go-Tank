...
protected:
	...
	void OnMessage(std::shared_ptr<olc::net::connection<GameMsg>> client, olc::net::message<GameMsg>& msg) override
	{
		...
		case GameMsg::FireBullet:
		{
			...
			msg >> desc;
			olc::net::message<GameMsg> msgFireBullet;
			msgFireBullet.header.id = GameMsg::FireBullet;
			msgFireBullet << desc;
			MessageAllClients(msgFireBullet, client);
			std::cout << "Firing!\n";
			break;
		}
		...
		{
			auto& pd = m_mapPlayerRoster[client->GetID()];
			std::cout << "[HITTED REMOVAL]:" + std::to_string(pd.nUniqueID) + "\n";
			m_mapPlayerRoster.erase(client->GetID());
			m_vGarbageIDs.push_back(client->GetID());
		}
